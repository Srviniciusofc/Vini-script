...
-- BLADE BALL - PARRY INTELIGENTE (MOBILE / DELTA)
...
-- CONFIG
local PARRY_DISTANCE = 18 -- ajuste fino
...
RunService.Heartbeat:Connect(function()
    ...
    for _,ball in pairs(Workspace:GetChildren()) do
        if ball:IsA("BasePart") and ball.Name == "Ball" then
            ...
            if target == LocalPlayer.Name or target == LocalPlayer then
...
-- TOGGLE
Tab:AddToggle({
    Title = "Parry Inteligente",
    Description = "Só bloqueia quando a bola mira em você",
    Default = false,
    Callback = function(state)
        AutoParry = state
    end
