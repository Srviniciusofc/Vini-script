...
-- BLADE BALL - AUTO PARRY (MOBILE / DELTA)
-- Bola em Workspace.Ball
...
-- CONFIG
local PARRY_DISTANCE = 18
local PARRY_COOLDOWN = 0.4

-- FUNÇÃO DE TOQUE (PARRY)
local function tap()
...
RunService.Heartbeat:Connect(function()
    ...
    -- VERIFICA SE A BOLA ESTÁ MIRANDO EM VOCÊ
    local target =
        ball:GetAttribute("target")
        or ball:GetAttribute("Target")
        or (ball:FindFirstChild("Target") and ball.Target.Value)

    if target ~= LocalPlayer and target ~= LocalPlayer.Name then return end
...
Tab:AddToggle({
    Title = "Auto Parry",
    Description = "Bloqueio automático (Blade Ball)",
    Default = false,
    Callback = function(state)
        AutoParry = state
    end
